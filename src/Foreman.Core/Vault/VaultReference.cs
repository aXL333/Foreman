...
/// <summary>
/// Parses and rewrites <c>{{vault:ORIGIN/FIELD}}</c> and selected
/// <c>{{vault:ORIGIN/ENTRY_ID/FIELD}}</c> references (e.g. <c>{{vault:github.com/password}}</c>). The
...
/// optional ENTRY_ID disambiguates multiple entries for one origin; FIELD is one of <see cref="VaultField"/>.
/// </summary>
public static partial class VaultReference
{
    ...
    [GeneratedRegex(@"\{\{vault:(?<origin>[A-Za-z0-9.\-]+(?::\d+)?)/(?:(?<entry>[A-Za-z0-9_-]{6,64})/)?(?<field>[A-Za-z]+)\}\}", RegexOptions.CultureInvariant)]
    ...
    public static bool HasPaymentCardReference(string? text) =>
        !string.IsNullOrEmpty(text) && Pattern().Matches(text).Any(m =>
            Enum.TryParse<VaultField>(m.Groups["field"].Value, ignoreCase: true, out var field) &&
            field is VaultField.CardholderName or VaultField.CardNumber or VaultField.CardExpiryMonth
                or VaultField.CardExpiryYear or VaultField.CardSecurityCode or VaultField.BillingAddress);
    ...
    public static string? Replace(string? text, Func<string, VaultField, string?> resolve) =>
        Replace(text, (origin, _, field) => resolve(origin, field));

    /// <summary>Selected-entry variant. <paramref name="entryId"/> is null for legacy two-segment references.</summary>
    public static string? Replace(string? text, Func<string, string?, VaultField, string?> resolve)
    {
        ...
        var output = Pattern().Replace(text, m =>
        {
            ...
            var entryId = m.Groups["entry"].Success ? m.Groups["entry"].Value : null;
            var value = resolve(m.Groups["origin"].Value, entryId, field);
