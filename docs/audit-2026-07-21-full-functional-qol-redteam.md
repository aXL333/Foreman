...
- **Guardian install no longer trusts a `--foreman` path.** The app passes only its live PID; the elevated Guardian resolves that process image itself and requires its own executable to occupy the canonical sibling `guardian` directory. Signed releases still require matching verified publishers. Unsigned development remains usable only through this live-launcher/layout route, rather than an unconditional unsigned-reference pass.
